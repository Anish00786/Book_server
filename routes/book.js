...
router.get('/', (req,res) => {
    ...
    router.get('/:id',(req,res)=>{
        Book.findById(req.params.id).then(books =>res.json(books)).catch(err => res.status(400).json({err}));
    })
...
Book.findByIdAndDelete(req.params.id).then((book)=>res.json({msg:'Deleted successfully'}))
})
